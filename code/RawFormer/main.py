...
def train_epoch(data_loader, model, lr,optim, device):
    ...
    for batch_x, batch_y, batch_meta in train_loader: 
        ...
        num_total += batch_size
        if ii == 0:
            print("batch_x.shape:",batch_x.shape) # ([32, 64600])
            print("batch_y.view(-1).shape:", batch_y.view(-1).shape) #([32])
        ii += 1
        batch_x = batch_x.to(device)
        batch_y = batch_y.view(-1).type(torch.int64).to(device)
        batch_out = model(batch_x,batch_y)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, default='RawNet2', choices=['RawNet2','RawFormer'])
    args = parser.parse_args()
    print("args.model_type:", args.model_type)
    
    if args.model_type == 'RawNet2':
    ...
    # Dataloader
    print("Loading data... from:", args.database_path, args.protocols_path)
